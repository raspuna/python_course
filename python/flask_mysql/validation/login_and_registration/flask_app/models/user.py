...
class User:
    ...
    def insert(cls, data):
        ...
        address1, address2, zipcode, state, 
        birthday, created_at, updated_at) 
        VALUES 
        (%(first_name)s, %(last_name)s, %(email)s, %(password)s, 
        %(address1)s, %(address2)s, %(zipcode)s, %(state)s, 
        %(birthday)s, NOW(), NOW()); 
        """
    ...
    def validate(user):
        ...
        if len(user['zipcode']) != 5:
            flash("Wrong zipcode", category='register')
            return False
        if len(user['address1']) < 1:
            flash("Address 1 field is empty", category='register')
            return False
        # birthday validtation is missing
